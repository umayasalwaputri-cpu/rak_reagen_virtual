...
# --- HALAMAN 2: APLIKASI UTAMA (Jika Sudah Login) ---
    ...
    with st.sidebar:
        ...
        # 🎯 MENU NAVIGASI TEMA DI SIDEBAR
        st.markdown("### 🗺️ Menu Instrumen")
        pilihan_halaman = st.radio(
            "Pilih Halaman Kerja:",
            ["🏠 Beranda Lab", "🧪 Rak Reagen Virtual"]
        )
        
        st.markdown("---")
        # Tombol Logout ditaruh di paling bawah sidebar
    ...
    # KONDISI A: JIKA MEMILIH BERANDA LAB
    if pilihan_halaman == "🏠 Beranda Lab":
        ...
        # Kartu Sambutan Estetik
        st.markdown("""
            <div style='background-color: #FFFFFF; border-left: 8px solid #0284C7; padding: 25px; border-radius: 8px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);'>
                <h4 style='color: #0369A1; margin: 0;'>Halo, Analis Kimia! 🧪</h4>
                <p style='color: #334155; margin-top: 12px; line-height: 1.6;'>
                ...
                <p style='color: #64748B; font-size: 14px; margin-top: 15px;'>
                    💡 <i>Tips: Silakan gunakan menu di <b>sisi kiri (sidebar)</b> untuk berpindah ke area Rak Reagen Virtual dan memulai pengujian sampel misterius Anda.</i>
                </p>
            </div>
        """, unsafe_allow_html=True)

    # KONDISI B: JIKA MEMILIH RAK REAGEN VIRTUAL
